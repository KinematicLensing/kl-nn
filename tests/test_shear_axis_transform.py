...
def test_clockwise_45_degree_major_axis_maps_positive_g2_to_g_plus(backend):
    """A +45 degree image-array angle is clockwise on the displayed image."""
    if backend == "torch":
        g1 = torch.tensor(0.0, dtype=torch.float64)
        g2 = torch.tensor(1.0, dtype=torch.float64)
        theta = torch.tensor(np.pi / 4, dtype=torch.float64)
        expected_plus = torch.tensor(1.0, dtype=torch.float64)
        expected_cross = torch.tensor(0.0, dtype=torch.float64)
        assert_close = torch.testing.assert_close
    else:
        g1 = np.asarray(0.0)
        g2 = np.asarray(1.0)
        theta = np.asarray(np.pi / 4)
        expected_plus = np.asarray(1.0)
        expected_cross = np.asarray(0.0)
