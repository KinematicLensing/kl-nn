...
\author{}
\date{Working-tree snapshot: 2026-09-07}
\begin{document}
\maketitle
\begin{center}
\small\textbf{Monitored-source fingerprint:} \code{8b596d7a94aed55e4b0712ac72a8662c4c82eca7f5c5f4f710a493a88251872a}
\end{center}
...
in \code{cosi}, matching the isotropic-orientation distribution. It is converted to
\code{sini = sqrt(1 - cosi**2)} only at the forward-renderer boundary. These facts follow directly from
...
\href{../arch/diagnostics/shear_bias_report.py}{\code{shear_bias_report.py}}. The cache
Slurm launcher requires one array task per partition by default and accepts
\code{ALLOW_PARTIAL_ARRAY=1} for a sparse resume of named task indices
(\href{../arch/cache_posteriors.slurm}{\code{cache_posteriors.slurm}}).
...
catalog selection function. \code{image_snr} is stratified over \([10,1000]\), and
\code{central_halpha_snr} over \([1,150]\). The latter two values survive into FITS
...
bidirectional feature-wise linear modulation (FiLM; \hyperlink{ref:perez2018}{Perez et al. 2018})
...
CCL checkpoint. The topology below is defined by \href{../arch/networks.py}{\code{ImgCNN}, \code{JointSpecCNN}, \code{MetadataMLP}, \code{SimpleFusionFeatureExtractor}, and \code{ImageSpectrumFilmFusion}}. Residual blocks follow
...
Spectrum & Conv pairs at 16, 32, 64, and 128 channels, each convolution followed by BatchNorm and ReLU; wavelength-only \code{1x2} max pooling after the 16- and 32-channel pairs only; two 256-channel convolutions; final \code{5x16} convolution spanning every fiber and remaining wavelength bin & 512 \\
...
reducing 64 bins to 16 before the final \code{5x16} kernel. It therefore models the
...
\subsection{Additive comparison architecture}

The concat encoder and nine-target hybrid flow remain the production NPE
path. \code{--arch concat} is the default of
\href{../arch/train_model.py}{\code{train_model.py}} and
\href{../arch/train_npe.slurm}{\code{train_npe.slurm}}. An additive alternative is
selected with \code{--arch comparison} and launched only from
\href{../arch/train_comparison_npe.slurm}{\code{train_comparison_npe.slurm}}. It does
...
\code{fiber_mask}, so a later data set with \(N\neq 5\) fibers can drop unoccupied
tokens without occupying a hardcoded major/minor slot.

The comparison encoder is
\href{../arch/comparison_arch.py}{\code{PhotometricShapeHead}, \code{SharedFiberSpectrumEncoder}, \code{PositionQueryFiberPool}, and \code{ComparisonFeatureExtractor}}. A small image CNN
maps the \code{48x48} cutout to 128 dimensions. Each fiber spectrum is encoded by
one shared 1-D CNN with the JointSpecCNN channel schedule
\code{16 -> 32 -> 64 -> 128 -> 256}, the same two wavelength-only pools (64 bins
...
\hyperlink{ref:vaswani2017}{Vaswani et al. 2017}) uses those queries against the spectral
...
pool. Concat's \href{../arch/networks.py}{\code{SimpleFusionFeatureExtractor}} still
rejects any False \code{fiber_mask} entry. Catalog metadata is the three oracle
...
channels-last layout is disabled because the spectral tower is \code{Conv1d}.
...
\href{../arch/comparison_arch.py}{\code{ComparisonKLNPE}} exposes the same \code{forward},
\code{posterior_log_prob}, and \code{sample} methods as concat \code{KLNPE}, so
\href{../arch/cache_posteriors.py}{\code{cache_posteriors.py}} selects the class from
\code{train.architecture} and does not grow a second sampling adapter. Training
logs \code{g_log_prob_mean} alongside the existing nuisance-component
diagnostics. `\code{--arch comparison_joint}`, launched from
\href{../arch/train_comparison_joint_npe.slurm}{\code{train_comparison_joint_npe.slurm}},
...
`\code{--arch kl_geom}`, launched from
\href{../arch/train_kl_geom_npe.slurm}{\code{train_kl_geom_npe.slurm}}, is a second
...
first-order \hyperlink{ref:xu2024}{Xu et al. (2024)} map in that fiber frame and rotates
...
\href{../arch/cache_posteriors.py}{\code{cache_posteriors.py}} still consumes composed
nine-target samples. Geometric training is eager: \code{torch.compile} of this
...
The Xu map remains marked \code{torch.compiler.disable}, and \(V_{\mathrm{circ}}\)
...
the flow. The NPE launcher passes \code{--freeze-feature-extractor}. Passing
\code{--no-freeze-feature-extractor} leaves those parameters trainable, keeps the
extractor in training mode so BatchNorm statistics update, places unfrozen
backbone weights in the NPE \code{shared} AdamW group, and, on more than one GPU,
...
\code{--no-image-spectrum-fusion} replaces the mix with the identity so NPE
...
\href{../arch/comparison_arch.py}{\code{ComparisonKLNPE}}; it does not load a CCL
backbone.
...
\href{../arch/train_npe.slurm}{\code{train_npe.slurm}},
\href{../arch/train_comparison_npe.slurm}{\code{train_comparison_npe.slurm}}, and the
...
Feature extractor & trainable & frozen by default (\code{--freeze-feature-extractor}) \\
Image–spectrum fusion & none; concat only & bidirectional FiLM on 512+512, identity-initialized, trainable by default (\code{--image-spectrum-fusion}) \\
\bottomrule
\end{tabularx}
\end{center}

The comparison NPE launcher
\href{../arch/train_comparison_npe.slurm}{\code{train_comparison_npe.slurm}} keeps the
...
checkpoint, forces \code{--no-freeze-feature-extractor}, omits FiLM flags, and
disables channels-last layout.
\href{../arch/train_comparison_joint_npe.slurm}{\code{train_comparison_joint_npe.slurm}}
is the same launcher with \code{--arch comparison_joint}.
\href{../arch/train_kl_geom_npe.slurm}{\code{train_kl_geom_npe.slurm}} is the same
100,000 / 10,000 from-scratch NPE launcher with \code{--arch kl_geom} and
\code{--no-compile}.
...
\href{../arch/model_registry.py}{\code{model_registry.py}}. Comparison runs additionally
snapshot \href{../arch/comparison_arch.py}{\code{comparison_arch.py}}. Geometric
\code{--arch kl_geom} runs snapshot \href{../arch/geom_arch.py}{\code{geom_arch.py}}. Concat
snapshots remain \code{networks.py} plus \code{circular_spline.py} only.
...
\hypertarget{ref:perez2018}{}%
\textbf{Perez, E., Strub, F., de Vries, H., Dumoulin, V., \& Courville, A. (2018).} FiLM: Visual reasoning with a general conditioning layer. \emph{AAAI}. \href{https://arxiv.org/abs/1709.07871}{arXiv:1709.07871}.\par
...
\hypertarget{ref:vaswani2017}{}%
\textbf{Vaswani, A., Shazeer, N., Parmar, N., Uszkoreit, J., Jones, L., Gomez, A. N., Kaiser, Ł., \& Polosukhin, I. (2017).} Attention is all you need. \emph{NeurIPS, 30}. \href{https://papers.nips.cc/paper_files/paper/2017/hash/3f5ee243547dee91fbd053c1c4a845aa-Abstract.html}{Paper}.\par
