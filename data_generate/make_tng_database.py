...
def load_fiducial_parameters(sample_name, ngal, rows_per_gal):
    ...
    # For TNG we store: g1, g2, theta_int, sini(cosi).
    total = ngal * rows_per_gal
    fids = np.zeros((total, 4), dtype=np.float32)
    ...
    for gal_idx in range(ngal):
        ...
        row_ids = np.arange(rows_per_gal, dtype=np.int64)
        ...
        if 'cosi' in df.columns:
            cosi = df['cosi'].to_numpy(dtype=np.float32)[:rows_per_gal]
        elif 'i' in df.columns:
            cosi = df['i'].to_numpy(dtype=np.float32)[:rows_per_gal]
        else:
            raise ValueError(f'Expected a cosi/i column in {sample_file}, found {list(df.columns)}')
        ...
        cosi = normalize('-11', cosi, (0.0, 1.0)).astype(np.float32)
