...
parser.add_argument('--no_psf', action='store_true', help='whether to generate fits without PSF convolution')
...
d = args.d
no_psf = args.no_psf
no_psf_str = '--no_psf' if no_psf else ''
...
for row in chunk:
    ...
    ID = int(ID)
    os.system(f"python {join(SCR_DIR, 'generate_fits.py')} -n={n} -d={d} -ID={ID} -g1={g1} -g2={g2} -theta_int={theta_int} -sini={sini} -v0={v0} -vcirc={vcirc} -rscale={rscale} -hlr={hlr} {no_psf_str}")