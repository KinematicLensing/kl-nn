...
def train_nn(rank: int, world_size: int, Model=ForkCNN, Trainer=CNNTrainer,
    ...
    train_ds, valid_ds, model, optimizer = load_train_objs(nfeatures, batch_size, world_size, Model, rank, epoch)
    model = model.to(rank)
...
def load_train_objs(nfeatures, batch_size, nGPUs, Model, rank, epoch=None):
