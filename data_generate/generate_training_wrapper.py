...
SAMP_FILE = '/xdisk/timeifler/wxs0703/kl_nn/samples/samples_train_low_g_1m.csv'

os.system(f"mkdir /xdisk/timeifler/wxs0703/kl_nn/train_data/temp_{n}")
...
for row in chunk:
    ...
    ID = int(ID)
    os.system(f"python generate_training_set.py -n={n} -isTrain={1} -ID={ID} -g1={g1} -g2={g2} -theta_int={theta_int} -sini={sini} -v0={v0} -vcirc={vcirc} -rscale={rscale} -hlr={hlr}")